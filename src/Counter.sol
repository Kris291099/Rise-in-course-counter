...
contract CounterContract {
    ...
    modifier onlyOwner() {
        require(
            msg.sender == owner,
            "Only the owner can increment or decrement the counter"
        );
        _;
    }
    ...
    function increment_counter() public onlyOwner {
        require(
            counter.number >= 0,
            "The counter number should not be negative"
        );
        counter.number += 1;
    }
    ...
    function decrement_counter() public onlyOwner {
        require(counter.number > 0, "Counter cannot be less than zero");
        counter.number -= 1;
    }
